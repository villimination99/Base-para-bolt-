...
(function() {
  ...
  document.querySelectorAll('.muscle-btn').forEach(function(btn) {
    btn.addEventListener('click', function() {
      ...
      /* Si alguien salta directo a un musculo antes de que el mapa haya
         entrado en pantalla, se pide ya: es la otra puerta de entrada. */
      if (window.__pedirMapa3D) window.__pedirMapa3D();
  ...
  /* CARGA PEREZOSA Y VIGILANTE DEL MAPA 3D
     ------------------------------------------------------------------
     El modulo del mapa son 520 KB (134 KB por la red). Bajarlos al abrir la
     pagina es pagarlos aunque el visitante lea el titular y se vaya, y
     ademas compiten con lo que si se esta viendo. Asi que se piden cuando
     el mapa se acerca a la pantalla -- o en cuanto alguien toca un boton de
     musculo, que es la otra forma de llegar a el.

     Esto solo actua en el tema, donde la seccion deja un #vi-p-cargas con
     las direcciones de assets/. En la version de un solo archivo, que se
     pega en una pagina, los scripts ya vienen puestos y aqui no hay nada
     que hacer: sin ese elemento, esta funcion se retira.

     Y el vigilante ya no es un reloj. Antes eran 15 segundos desde el
     arranque -- tarde cuando el modulo no iba a ejecutarse nunca, y pronto
     con una conexion mala. Ahora se pregunta lo que de verdad importa, y en
     el momento exacto:

       - Si el navegador no entiende modulos, 'noModule' no existe en los
         <script>. Se sabe ANTES de pedir nada, sin esperar ni un
         milisegundo, y se muestra el respaldo.
       - Si el archivo no llega, salta su onerror.
       - Si llega y se ejecuta, salta su onload; 600 ms despues el modulo ya
         ha tenido tiempo de crear el renderizador y marcar su bandera. Si
         no la marco, es que fallo por dentro.

     Ninguna de las tres ramas depende de lo rapida que sea la red. */
  (function cargaDelMapa3D() {
    var cfg = document.getElementById('vi-p-cargas');
    if (!cfg) return;                       // version de un solo archivo
    var url = cfg.getAttribute('data-mapa3d');
    var lienzo = document.getElementById('muscle-canvas');
    if (!url || !lienzo) return;
    ...
    function pedir() {
      if (pedido) return;
      pedido = true;
      if (!('noModule' in document.createElement('script'))) { respaldo(); return; }
      var s = document.createElement('script');
      s.type = 'module';
      s.src = url;
      s.onerror = respaldo;
      s.onload = function () {
        setTimeout(function () { if (!window.__mm3dStarted) respaldo(); }, 600);
      };
      document.head.appendChild(s);
    }
    window.__pedirMapa3D = pedir;           // lo usan los botones de musculo

    if (!('IntersectionObserver' in window)) { pedir(); return; }
    var ojo = new IntersectionObserver(function (entradas) {
      for (var i = 0; i < entradas.length; i++) {
        if (entradas[i].isIntersecting) { ojo.disconnect(); pedir(); return; }
      }
    }, { rootMargin: '500px 0px' });        // arranca antes de que se vea
    ojo.observe(lienzo);
  })();

  /* CARGA PEREZOSA DE LAS GRAFICAS
     Chart.js son otros 164 KB (56 KB por la red) para dos graficas que estan
     bien abajo. Se pide cuando la seccion de nutricion se acerca. Quien
     necesita dibujar espera en conChart(), que ejecuta al momento si la
     libreria ya esta -- que es lo que pasa en la version de un solo
     archivo, donde viene puesta desde el principio. */
  var __colaChart = [];
  window.conChart = function (fn) {
    if (window.Chart) { fn(); return; }
    __colaChart.push(fn);
  };
  (function cargaDeLasGraficas() {
    var cfg = document.getElementById('vi-p-cargas');
    if (!cfg) return;
    var url = cfg.getAttribute('data-chart');
    var ancla = document.getElementById('nutrition') || document.getElementById('macro-chart');
    if (!url || !ancla) return;
    ...
    function pedir() {
      ...
      s.src = url;
      s.onload = function () {
        var cola = __colaChart; __colaChart = [];
        for (var i = 0; i < cola.length; i++) { try { cola[i](); } catch (e) { console.warn('grafica:', e.message); } }
      };
      /* Si no llega, cada quien se queda con su aviso en su sitio: el hub ya
         comprueba que Chart exista antes de dibujar. */
      s.onerror = function () { __colaChart = []; };
      document.head.appendChild(s);
    }
    if (!('IntersectionObserver' in window)) { pedir(); return; }
    var ojo = new IntersectionObserver(function (entradas) {
  ...
  function applyLang(l) {
    ...
    if (D) {
      ...
      /* D es I18N[l], asi que D.note YA viene en el idioma de destino. Pasarla
         otra vez por TX() la buscaba en el diccionario como si fuera espanol,
         no la encontraba -- claro -- y la apuntaba en __i18nMiss. El texto en
         pantalla salia bien, pero el informe de cadenas pendientes daba un
         falso positivo en cada idioma. Eso es justo lo que vuelve inservible
         un informe: con ruido fijo, nadie lo mira. Por eso llevaba ahi sin que
         nadie lo usara. */
  ...
  (function initNutrition() {
    try {
      ...
      /* conChart dibuja ya si la libreria esta, y si se esta bajando de
         forma perezosa deja el trabajo en cola hasta que llegue.
         (Aqui habia ademas un "if (typeof Chart === 'undefined') return;"
         DENTRO de un "if (typeof Chart !== 'undefined')": codigo muerto, y
         de los peligrosos, porque ese return no abortaba la grafica sino
         initNutrition entera -- el registro de comidas, el buscador y la
         camara se habrian ido con el.) */
      window.conChart(function () {
  ...
  (function initProgress() {
    try {
      ...
      /* Igual que el anillo de macros: si Chart.js se esta bajando de forma
         perezosa, esto espera en la cola en vez de rendirse. El try/catch
         se repite dentro porque el de fuera ya habra terminado cuando la
         libreria llegue. */
      window.conChart(function () {
       try {
      ...
      document.querySelectorAll('.period-tab').forEach(function(tab) {
        tab.addEventListener('click', function() {
         ...
         console.warn('Progress error:', e.message);
         var pc2 = document.getElementById('progress-chart');
         if (pc2 && pc2.parentElement) pc2.parentElement.innerHTML = '<div class="module-error">📊 ' + T('err.chart', 'Gráfica no disponible.') + '</div>';
       }
      });
    } catch(err) {
